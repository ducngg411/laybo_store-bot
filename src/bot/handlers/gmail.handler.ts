...
const userSessions = new Map<number, GmailSession>();
...
function generateQuantityKeyboard(stock: number, showMaxButton: boolean = false) {
    ...
    if (stock <= 0) {
        ...
        buttons.push([Markup.button.callback('↩️ Quay lại', CALLBACK_ACTIONS.GMAIL_GO_BACK_TO_PLANS)]);
    } else if (stock <= 5) {
        ...
        for (let i = 1; i <= stock; i++) {
            qtyButtons.push(Markup.button.callback(
                i.toString(),
                `${CALLBACK_ACTIONS.GMAIL_QTY_PREFIX}${i}`
            ));
        }
        ...
        if (showMaxButton) {
            ...
            buttons.push([
                Markup.button.callback(
                    `✅ Mua ${stock} (tối đa)`,
                    CALLBACK_ACTIONS.GMAIL_QTY_MAX
                ),
            ]);

            // Show only 1 and 2 for quick selection
            const quickButtons = [];
            if (stock >= 1) quickButtons.push(Markup.button.callback('1', `${CALLBACK_ACTIONS.GMAIL_QTY_PREFIX}1`));
            if (stock >= 2) quickButtons.push(Markup.button.callback('2', `${CALLBACK_ACTIONS.GMAIL_QTY_PREFIX}2`));
            ...
            buttons.push([
                Markup.button.callback('1', `${CALLBACK_ACTIONS.GMAIL_QTY_PREFIX}1`),
                Markup.button.callback('2', `${CALLBACK_ACTIONS.GMAIL_QTY_PREFIX}2`),
                Markup.button.callback('3', `${CALLBACK_ACTIONS.GMAIL_QTY_PREFIX}3`),
            ]);
            buttons.push([
                Markup.button.callback('4', `${CALLBACK_ACTIONS.GMAIL_QTY_PREFIX}4`),
                Markup.button.callback('5', `${CALLBACK_ACTIONS.GMAIL_QTY_PREFIX}5`),
            ]);
            buttons.push([
                Markup.button.callback('✏️ Nhập số khác', CALLBACK_ACTIONS.GMAIL_QTY_CUSTOM),
                Markup.button.callback(`✅ Mua ${stock} (tối đa)`, CALLBACK_ACTIONS.GMAIL_QTY_MAX),
            ]);
            buttons.push([Markup.button.callback('↩️ Quay lại', CALLBACK_ACTIONS.GMAIL_GO_BACK_TO_PLANS)]);
        }
    }
...
export async function handleGmailSelect(ctx: Context) {
    ...
    // Show plan selection
    const product = await productRepository.findByCode(PRODUCT_CODES.GMAIL);
    if (!product) {
    ...
    const buttons = variants.map((v) =>
        Markup.button.callback(
            ...
            `${CALLBACK_ACTIONS.GMAIL_PLAN_PREFIX}${v.code}`
        )
    );

    // Add back button
    buttons.push(Markup.button.callback('↩️ Quay lại', CALLBACK_ACTIONS.GMAIL_GO_BACK_TO_MAIN));

    const keyboard = Markup.inlineKeyboard(buttons, { columns: 1 });

    userSessions.set(userId, { step: 'select_plan' });

    await ctx.reply('📧 *Gmail Plus 1 tháng (Bảo hành full)*\n\nVui lòng chọn gói:', {
        parse_mode: 'Markdown',
        ...keyboard,
    });
}

export async function handleGmailPlanSelect(ctx: Context, variantCode: string) {
    const userId = ctx.from?.id;
    if (!userId) return;
...
export async function handleGmailQuantitySelect(ctx: Context, quantity: number, isFromTextInput: boolean = false) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    if (quantity > currentStock) {
        ...
        const message = BOT_MESSAGES.GMAIL_INSUFFICIENT_STOCK(quantity, currentStock);
    ...
    await createGmailOrder(ctx, quantity, session, isFromTextInput);
}

/**
 * Handle "Mua tối đa" button - buy max available stock
 */
export async function handleGmailQuantityMax(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    await createGmailOrder(ctx, maxStock, session);
}

/**
 * Handle custom quantity input from text message
 */
export async function handleGmailQuantityCustom(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    const currentStock = session.currentStock || 0;
    const maxAllowed = Math.min(currentStock, LIMITS.GMAIL_QTY_MAX);

    await ctx.editMessageText(
        `📝 Vui lòng nhập số lượng tài khoản (${LIMITS.GMAIL_QTY_MIN}-${maxAllowed}):`,
        { parse_mode: 'Markdown' }
    );
}
 ...
 */
export async function handleGmailQuantityInput(ctx: Context, text: string) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    if (isNaN(qty)) {
        ...
        await ctx.reply(
            BOT_MESSAGES.GMAIL_INVALID_QUANTITY(LIMITS.GMAIL_QTY_MIN, maxAllowed),
            {
                parse_mode: 'Markdown',
                ...keyboard,
            }
    ...
    if (qty < LIMITS.GMAIL_QTY_MIN || qty > LIMITS.GMAIL_QTY_MAX) {
        // Out of general bounds - use reply instead of edit
        const keyboard = generateQuantityKeyboard(currentStock, false);
        await ctx.reply(
            BOT_MESSAGES.GMAIL_INVALID_QUANTITY(LIMITS.GMAIL_QTY_MIN, LIMITS.GMAIL_QTY_MAX),
            {
                parse_mode: 'Markdown',
                ...keyboard,
            }
    ...
    if (qty > currentStock) {
        ...
        await ctx.reply(
            BOT_MESSAGES.GMAIL_INSUFFICIENT_STOCK(qty, currentStock),
            {
                parse_mode: 'Markdown',
                ...keyboard,
            }
    ...
    await handleGmailQuantitySelect(ctx, qty, true); // true = from text input
}

/**
 * Handle "Quay lại" button - go back to main menu
 */
export async function handleGmailGoBackToMain(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;
 ...
 */
export async function handleGmailGoBackToPlans(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;

    await ctx.answerCbQuery('↩️ Quay lại');
    await handleGmailSelect(ctx);
}

/**
 * Create Gmail order after validation
 */
async function createGmailOrder(ctx: Context, quantity: number, session: GmailSession, isFromTextInput: boolean = false) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    try {
        ...
        const order = await orderService.createOrder({
            ...
            productCode: PRODUCT_CODES.GMAIL,
        ...
        logger.info({
            ...
            quantity,
            qrMessageId: sentMessage.message_id
        }, 'Gmail order created, QR sent with messageId saved');
    } catch (error) {
        logger.error({ error, userId, quantity }, 'Failed to create Gmail order');
        ...
        if (error instanceof Error && error.message === 'USER_HAS_ACTIVE_ORDER') {
            ...
            await handleGmailPlanSelect(ctx, session.variantCode!);
        } else {
