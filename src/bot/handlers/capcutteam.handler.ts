...
const userSessions = new Map<number, CapcutTeamSession>();
...
function generateQuantityKeyboard(stock: number, showMaxButton: boolean = false) {
    ...
    if (stock <= 0) {
        ...
        buttons.push([Markup.button.callback('↩️ Quay lại', CALLBACK_ACTIONS.CAPCUTTEAM_GO_BACK_TO_PLANS)]);
    } else if (stock <= 5) {
        ...
        for (let i = 1; i <= stock; i++) {
            qtyButtons.push(Markup.button.callback(
                i.toString(),
                `${CALLBACK_ACTIONS.CAPCUTTEAM_QTY_PREFIX}${i}`
            ));
        }
        ...
        if (showMaxButton) {
            ...
            buttons.push([
                Markup.button.callback(
                    `✅ Mua ${stock} (tối đa)`,
                    CALLBACK_ACTIONS.CAPCUTTEAM_QTY_MAX
                ),
            ]);

            // Show only 1 and 2 for quick selection
            const quickButtons = [];
            if (stock >= 1) quickButtons.push(Markup.button.callback('1', `${CALLBACK_ACTIONS.CAPCUTTEAM_QTY_PREFIX}1`));
            if (stock >= 2) quickButtons.push(Markup.button.callback('2', `${CALLBACK_ACTIONS.CAPCUTTEAM_QTY_PREFIX}2`));
            ...
            buttons.push([
                Markup.button.callback('1', `${CALLBACK_ACTIONS.CAPCUTTEAM_QTY_PREFIX}1`),
                Markup.button.callback('2', `${CALLBACK_ACTIONS.CAPCUTTEAM_QTY_PREFIX}2`),
                Markup.button.callback('3', `${CALLBACK_ACTIONS.CAPCUTTEAM_QTY_PREFIX}3`),
            ]);
            buttons.push([
                Markup.button.callback('4', `${CALLBACK_ACTIONS.CAPCUTTEAM_QTY_PREFIX}4`),
                Markup.button.callback('5', `${CALLBACK_ACTIONS.CAPCUTTEAM_QTY_PREFIX}5`),
            ]);
            buttons.push([
                Markup.button.callback('✏️ Nhập số khác', CALLBACK_ACTIONS.CAPCUTTEAM_QTY_CUSTOM),
                Markup.button.callback(`✅ Mua ${stock} (tối đa)`, CALLBACK_ACTIONS.CAPCUTTEAM_QTY_MAX),
            ]);
            buttons.push([Markup.button.callback('↩️ Quay lại', CALLBACK_ACTIONS.CAPCUTTEAM_GO_BACK_TO_PLANS)]);
        }
    }
...
export async function handleCapcutTeamSelect(ctx: Context) {
    ...
    // Show plan selection
    const product = await productRepository.findByCode(PRODUCT_CODES.CAPCUTTEAM);
    if (!product) {
    ...
    const buttons = variants.map((v) =>
        Markup.button.callback(
            ...
            `${CALLBACK_ACTIONS.CAPCUTTEAM_PLAN_PREFIX}${v.code}`
        )
    );

    // Add back button
    buttons.push(Markup.button.callback('↩️ Quay lại', CALLBACK_ACTIONS.CAPCUTTEAM_GO_BACK_TO_MAIN));

    const keyboard = Markup.inlineKeyboard(buttons, { columns: 1 });

    userSessions.set(userId, { step: 'select_plan' });

    await ctx.reply('🎬 *CapcutTeam Pro 1 Tuần (Bảo hành full)*\n\nVui lòng chọn gói:', {
        parse_mode: 'Markdown',
        ...keyboard,
    });
}

export async function handleCapcutTeamPlanSelect(ctx: Context, variantCode: string) {
    const userId = ctx.from?.id;
    if (!userId) return;
...
export async function handleCapcutTeamQuantitySelect(ctx: Context, quantity: number, isFromTextInput: boolean = false) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    if (quantity > currentStock) {
        ...
        const message = BOT_MESSAGES.CAPCUTTEAM_INSUFFICIENT_STOCK(quantity, currentStock);
    ...
    await createCapcutTeamOrder(ctx, quantity, session, isFromTextInput);
}

/**
 * Handle "Mua tối đa" button - buy max available stock
 */
export async function handleCapcutTeamQuantityMax(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    await createCapcutTeamOrder(ctx, maxStock, session);
}

/**
 * Handle custom quantity input from text message
 */
export async function handleCapcutTeamQuantityCustom(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    const currentStock = session.currentStock || 0;
    const maxAllowed = Math.min(currentStock, 100); // Using same max as Netflix

    await ctx.editMessageText(
        `📝 Vui lòng nhập số lượng tài khoản (1-${maxAllowed}):`,
        { parse_mode: 'Markdown' }
    );
}
 ...
 */
export async function handleCapcutTeamQuantityInput(ctx: Context, text: string) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    const currentStock = session.currentStock || 0;
    const maxAllowed = Math.min(currentStock, LIMITS.CAPCUTTEAM_QTY_MAX);
    ...
    if (isNaN(qty)) {
        ...
        await ctx.reply(
            BOT_MESSAGES.CAPCUTTEAM_INVALID_QUANTITY(LIMITS.CAPCUTTEAM_QTY_MIN, maxAllowed),
            {
                parse_mode: 'Markdown',
                ...keyboard,
            }
    ...
    if (qty < LIMITS.CAPCUTTEAM_QTY_MIN || qty > LIMITS.CAPCUTTEAM_QTY_MAX) {
        // Out of general bounds - use reply instead of edit
        const keyboard = generateQuantityKeyboard(currentStock, false);
        await ctx.reply(
            BOT_MESSAGES.CAPCUTTEAM_INVALID_QUANTITY(LIMITS.CAPCUTTEAM_QTY_MIN, LIMITS.CAPCUTTEAM_QTY_MAX),
            {
                parse_mode: 'Markdown',
                ...keyboard,
            }
    ...
    if (qty > currentStock) {
        ...
        await ctx.reply(
            BOT_MESSAGES.CAPCUTTEAM_INSUFFICIENT_STOCK(qty, currentStock),
            {
                parse_mode: 'Markdown',
                ...keyboard,
            }
    ...
    await handleCapcutTeamQuantitySelect(ctx, qty, true); // true = from text input
}

/**
 * Handle "Quay lại" button - go back to main menu
 */
export async function handleCapcutTeamGoBackToMain(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;
 ...
 */
export async function handleCapcutTeamGoBackToPlans(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;

    await ctx.answerCbQuery('↩️ Quay lại');
    await handleCapcutTeamSelect(ctx);
}

/**
 * Create CapcutTeam order after validation
 */
async function createCapcutTeamOrder(ctx: Context, quantity: number, session: CapcutTeamSession, isFromTextInput: boolean = false) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    try {
        ...
        const order = await orderService.createOrder({
            ...
            productCode: PRODUCT_CODES.CAPCUTTEAM,
        ...
        logger.info({
            ...
            quantity,
            qrMessageId: sentMessage.message_id
        }, 'CapcutTeam order created, QR sent with messageId saved');
    } catch (error) {
        logger.error({ error, userId, quantity }, 'Failed to create CapcutTeam order');
        ...
        if (error instanceof Error && error.message === 'USER_HAS_ACTIVE_ORDER') {
            ...
            await handleCapcutTeamPlanSelect(ctx, session.variantCode!);
        } else {
