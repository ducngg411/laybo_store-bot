...
const userSessions = new Map<number, Veo3Session>();
...
function generateQuantityKeyboard(stock: number, showMaxButton: boolean = false) {
    ...
    if (stock <= 0) {
        ...
        buttons.push([Markup.button.callback('↩️ Quay lại', CALLBACK_ACTIONS.VEO3_GO_BACK_TO_PLANS)]);
    } else if (stock <= 5) {
        ...
        for (let i = 1; i <= stock; i++) {
            qtyButtons.push(Markup.button.callback(
                i.toString(),
                `${CALLBACK_ACTIONS.VEO3_QTY_PREFIX}${i}`
            ));
        }
        ...
        if (showMaxButton) {
            ...
            buttons.push([
                Markup.button.callback(
                    `✅ Mua ${stock} (tối đa)`,
                    CALLBACK_ACTIONS.VEO3_QTY_MAX
                ),
            ]);

            // Show only 1 and 2 for quick selection
            const quickButtons = [];
            if (stock >= 1) quickButtons.push(Markup.button.callback('1', `${CALLBACK_ACTIONS.VEO3_QTY_PREFIX}1`));
            if (stock >= 2) quickButtons.push(Markup.button.callback('2', `${CALLBACK_ACTIONS.VEO3_QTY_PREFIX}2`));
            ...
            buttons.push([
                Markup.button.callback('1', `${CALLBACK_ACTIONS.VEO3_QTY_PREFIX}1`),
                Markup.button.callback('2', `${CALLBACK_ACTIONS.VEO3_QTY_PREFIX}2`),
                Markup.button.callback('3', `${CALLBACK_ACTIONS.VEO3_QTY_PREFIX}3`),
            ]);
            buttons.push([
                Markup.button.callback('4', `${CALLBACK_ACTIONS.VEO3_QTY_PREFIX}4`),
                Markup.button.callback('5', `${CALLBACK_ACTIONS.VEO3_QTY_PREFIX}5`),
            ]);
            buttons.push([
                Markup.button.callback('✏️ Nhập số khác', CALLBACK_ACTIONS.VEO3_QTY_CUSTOM),
                Markup.button.callback(`✅ Mua ${stock} (tối đa)`, CALLBACK_ACTIONS.VEO3_QTY_MAX),
            ]);
            buttons.push([Markup.button.callback('↩️ Quay lại', CALLBACK_ACTIONS.VEO3_GO_BACK_TO_PLANS)]);
        }
    }
...
export async function handleVeo3Select(ctx: Context) {
    ...
    // Show plan selection
    const product = await productRepository.findByCode(PRODUCT_CODES.VEO3);
    if (!product) {
    ...
    const buttons = variants.map((v) =>
        Markup.button.callback(
            ...
            `${CALLBACK_ACTIONS.VEO3_PLAN_PREFIX}${v.code}`
        )
    );

    // Add back button
    buttons.push(Markup.button.callback('↩️ Quay lại', CALLBACK_ACTIONS.VEO3_GO_BACK_TO_MAIN));
...
export async function handleVeo3PlanSelect(ctx: Context, variantCode: string) {
    const userId = ctx.from?.id;
    if (!userId) return;
...
export async function handleVeo3QuantitySelect(ctx: Context, quantity: number, isFromTextInput: boolean = false) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    if (quantity > currentStock) {
        ...
        const message = BOT_MESSAGES.VEO3_INSUFFICIENT_STOCK(quantity, currentStock);
    ...
    await createVeo3Order(ctx, quantity, session, isFromTextInput);
}

/**
 * Handle "Mua tối đa" button - buy max available stock
 */
export async function handleVeo3QuantityMax(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    await createVeo3Order(ctx, maxStock, session);
}

/**
 * Handle custom quantity input from text message
 */
export async function handleVeo3QuantityCustom(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    const currentStock = session.currentStock || 0;
    const maxAllowed = Math.min(currentStock, LIMITS.VEO3_QTY_MAX);

    await ctx.editMessageText(
        `📝 Vui lòng nhập số lượng tài khoản (${LIMITS.VEO3_QTY_MIN}-${maxAllowed}):`,
        { parse_mode: 'Markdown' }
    );
}
 ...
 */
export async function handleVeo3QuantityInput(ctx: Context, text: string) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    if (isNaN(qty)) {
        ...
        await ctx.reply(
            BOT_MESSAGES.VEO3_INVALID_QUANTITY(LIMITS.VEO3_QTY_MIN, maxAllowed),
            {
                parse_mode: 'Markdown',
                ...keyboard,
            }
    ...
    if (qty < LIMITS.VEO3_QTY_MIN || qty > LIMITS.VEO3_QTY_MAX) {
        // Out of general bounds - use reply instead of edit
        const keyboard = generateQuantityKeyboard(currentStock, false);
        await ctx.reply(
            BOT_MESSAGES.VEO3_INVALID_QUANTITY(LIMITS.VEO3_QTY_MIN, LIMITS.VEO3_QTY_MAX),
            {
                parse_mode: 'Markdown',
                ...keyboard,
            }
    ...
    if (qty > currentStock) {
        ...
        await ctx.reply(
            BOT_MESSAGES.VEO3_INSUFFICIENT_STOCK(qty, currentStock),
            {
                parse_mode: 'Markdown',
                ...keyboard,
            }
    ...
    await handleVeo3QuantitySelect(ctx, qty, true); // true = from text input
}

/**
 * Handle "Quay lại" button - go back to main menu
 */
export async function handleVeo3GoBackToMain(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;
 ...
 */
export async function handleVeo3GoBackToPlans(ctx: Context) {
    const userId = ctx.from?.id;
    if (!userId) return;

    await ctx.answerCbQuery('↩️ Quay lại');
    await handleVeo3Select(ctx);
}

/**
 * Create Veo3 order after validation
 */
async function createVeo3Order(ctx: Context, quantity: number, session: Veo3Session, isFromTextInput: boolean = false) {
    const userId = ctx.from?.id;
    if (!userId) return;
    ...
    try {
        ...
        const order = await orderService.createOrder({
            ...
            productCode: PRODUCT_CODES.VEO3,
        ...
        logger.info({
            ...
            quantity,
            qrMessageId: sentMessage.message_id
        }, 'Veo3 order created, QR sent with messageId saved');
    } catch (error) {
        logger.error({ error, userId, quantity }, 'Failed to create Veo3 order');
        ...
        if (error instanceof Error && error.message === 'USER_HAS_ACTIVE_ORDER') {
            ...
            await handleVeo3PlanSelect(ctx, session.variantCode!);
        } else {
